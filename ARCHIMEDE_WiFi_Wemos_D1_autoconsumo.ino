...
// 7) SULLE USCITE COLLOCARE IN PROVA 2 LED DI CONTROLLO, OPPURE 2 RELE' A STATO SOLIDO CHE HANNO UN LED INCORPORATO (SI ILLUMINA QUANDO E' ATTIVATO).
...
void loop()
{
  ...
  if (tempo_adesso > timer_rele_vecchio + 6000)     // sono passati 10 secondi (solo per prove tecniche):
  {
    timer_rele_vecchio = tempo_adesso;
    if (rele_on_off == 0 && potenza_watt_produzione > potenza_watt_consumi + 300)     // il relè ON-OFF si attiva solo quando c'è un surplus di 300 watt:
    {
      rele_on_off = 1;     // HIGH:
