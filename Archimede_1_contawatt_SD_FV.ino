/* PROGRAMMA CONTAWATT per contatore digitale e memoria SD. 
 Versione per display 2x16 SERIALE (A4 e A5)
 Allaccio contatori a A1 (produzione)
 autore SoftPlus Consumo Zero - email: luigi.marchi.faenza@gmail.com  
 Versione software del 21 luglio 2017:
 //
 //
...
void loop() {
  ...
  if (tempo_adesso_produzione > vecchio_millis_produzione + 100)   // bisogna che siano passati almeno 100 millisecondi dall'ultima lettura:
  {
    ...
    if (corrente_millis_minuti > vecchio_millis_minuti + 300000)   // significa che sono passati 5 minuti (60.000 x 5 = 300000):
    {
      ...
      //
      // aggiornamento orario ogni 5 minuti:
      ...
      { 
        contawatt_m1 = m1*1000/impulsi_kwh;
        file.println(contawatt_m1*12);   // Scrivo i watt di consumo ( valido per 5 minuti),  rapportato ad ora:   
      ...
      { 
        ...
        if (corrente_millis_giorni > vecchio_millis_giorni + 86400000)   // 24 ore (3,6 milioni x 24 = 86400000): 
        {
          ...
          // impulsi_30g = g1+g2+g3+g4+g5+g6+g7+g8+g9+g10+g11+g12+g13+g14+g15+g16+g17+g18+g19+g20+g21+g22+g23+g24+g25+g26+g27+g28+g29+g30; 
          //
          file = SD.open("produzione_24h.txt", FILE_WRITE); //File in scrittura:  
          if (file) //Se il file è stato aperto correttamente:
          {    
  ...
  if (conta1 > 15000)
  {
    contawatt = impulsi_produzione*1000/impulsi_kwh;   // perchè abbiamo 3200 impulsi per Kwh: 
    if (contawatt > 99999)
    {
      lcd.setCursor(0, 0);
      lcd.print("W_FV   Kwh_Produ");  
      lcd.setCursor(0, 1);
      lcd.print("                "); 
      lcd.setCursor(1, 1);  
      lcd.print(val_prod);
